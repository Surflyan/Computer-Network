...
//Web Cache 的实现
struct WebCACHE {
	HttpHeader htphed;     //客户端请求头部
	char buffer[MAXSIZE];  //存储服务器返回数据
	char date[DATELENGTH]; //存储报文更新更新时间
	WebCACHE() {
...
int cacheNum = 0;               //已缓存数量
WebCACHE cache[CACHE_MAXSIZE];  //存储缓存数据

BOOL InitSocket();
void ParseHttpHead(char *buffer, HttpHeader * httpHeader);
BOOL ConnectToServer(SOCKET *serverSocket, char *host);
unsigned int __stdcall ProxyThread(LPVOID lpParameter);
int Cache_find(WebCACHE *cache, HttpHeader htp);

//代理相关参数 
SOCKET ProxyServer;
sockaddr_in ProxyServerAddr;
const int ProxyPort = 1028;
//禁用网站
char  *disabledHost[10] = { "today.hit.edu.cn", "alibaba.com" };
const int host_number = 0;
...
//网站诱导
char *induceSite = "sina.com.cn";
char *targetSite = "jwts.hit.edu.cn";
...
unsigned int __stdcall ProxyThread(LPVOID lpParameter) {
	...
	if (isUpdate) {
		...
		{
			memcpy(&(cache[cacheNum%CACHE_MAXSIZE].htphed.host), httpHeader->host, strlen(httpHeader->host));
			memcpy(&(cache[cacheNum%CACHE_MAXSIZE].htphed.method), httpHeader->method, strlen(httpHeader->method));
			memcpy(&(cache[cacheNum%CACHE_MAXSIZE].htphed.url), httpHeader->url, strlen(httpHeader->url));
			memcpy(&(cache[cacheNum%CACHE_MAXSIZE].buffer), Buffer, strlen(Buffer));
			memcpy(&(cache[cacheNum%CACHE_MAXSIZE].date), dada, strlen(dada));
			cacheNum++;
		}
	}
...
//判断两个报文是否相同
BOOL Isequal(HttpHeader htp1, HttpHeader htp2)
{
